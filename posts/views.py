...
class HomeView(generic.TemplateView):
    ...
    def get_hero_display_posts(self, **kwargs):
        context = {
            "slider_posts": Post.objects.all().order_by("-createdAt"),
            "top_hero": Post.objects.all().order_by("?")[:1],
            "bottom": Post.objects.all().order_by("?")[:2],
            "top_posts": Post.objects.filter(topic__is_top_story=True).all()[:10],
            "best_category": Post.objects.is_popular(view_limit=100).all().order_by("?").first(),
    ...
    def get_context_data(self, **kwargs):
        ...
        context['best_posts'] = Post.objects.is_popular(view_limit=50).order_by("-createdAt").all()
        context['trending'] = Post.objects.is_popular(view_limit=500).all().order_by("?")
        return context
...
class PostDetailView(generic.TemplateView):
    ...
    def get_object(self, **kwargs):
        return get_object_or_404(self.queryset, pk=kwargs.get("pk"), slug=kwargs.get("slug"))
    
    def next_item_in_posts(self, **kwargs):
        next = get_next_or_prev(self.queryset.objects.all().order_by("pk"), self.get_object(**kwargs), "next")
        return next
    
    def prev_item_in_posts(self, **kwargs):
        prev = get_next_or_prev(self.queryset.objects.all().order_by("pk"), self.get_object(**kwargs), "prev")
        return prev
    
    def related_posts(self, **kwargs):
        return self.queryset.objects.filter(
            topic__sub_category__name__exact=self.get_object(**kwargs).topic.sub_category.name
            ).all().exclude(id=self.get_object(**kwargs).pk).order_by("?")[:6]
